...
export const ProfileCourseAssignmentDetails: FC<{
  ...
  if (!courseLogic.haveSubmittedAssignmentFiles() && isPast(courseLogic.getDeadlineDate())) {
      ...
      <Stack>
        <KeyValue title="Dosyalar">
          <Wrap spacing={2}>
            {courseLogic
              .course
              .assignmentFiles?.map(file => <CourseAssignmentFileButton key={file.id} file={file} />)}
          </Wrap>
        </KeyValue>
        ...
        {courseLogic.haveSubmittedAssignmentFiles() ? (
          <KeyValue title="Ödev dosyalarını gönderdiniz.">
            <HStack>
              <Text>Dosyalar elimize ulaştı. Aşağıda belirtilen tarihe kadar değerlendirip sonucu size bildireceğiz.</Text>
              <Text>{formatDate(courseLogic.getEvaluationDate(), 'dd MMMM yyyy', locale)}</Text>
            </HStack>
          </KeyValue>
        ) : (
