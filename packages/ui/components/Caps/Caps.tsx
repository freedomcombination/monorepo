...
export const Caps = forwardRef<HTMLDivElement, CapsProps>(function Caps(
  { imageParams, hideLogo = false, hasRandomImage, ...rest },
  ref,
) {
  const {
    title,
        ...
        {dimensions.width > 0 ? (
          <Flex boxSize={'full'} bg={bg} pos={'relative'}>
            ...
            <Stack
              ...
              left={`${
                flip ? dimensions.image + dimensions.shape : dimensions.padding
              }px`}
              right={`${
                flip ? dimensions.padding : dimensions.image + dimensions.shape
              }px`}
